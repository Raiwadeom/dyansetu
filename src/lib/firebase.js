...
/* Trimmed defensively: these are typed into a hosting dashboard by hand, and
   ...
   bundle. It has already broken an upload cloud_name and a token project-id
   check this way, so every dashboard-entered value gets the same treatment
   here rather than waiting for each one to fail on its own. */
const trimmed = (value) => (typeof value === "string" ? value.trim() : value);

const config = {
  apiKey: trimmed(import.meta.env.VITE_FIREBASE_API_KEY),
  authDomain: trimmed(import.meta.env.VITE_FIREBASE_AUTH_DOMAIN),
  projectId: trimmed(import.meta.env.VITE_FIREBASE_PROJECT_ID),
  storageBucket: trimmed(import.meta.env.VITE_FIREBASE_STORAGE_BUCKET),
  messagingSenderId: trimmed(import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID),
  appId: trimmed(import.meta.env.VITE_FIREBASE_APP_ID),
};
